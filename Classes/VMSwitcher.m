...
@interface VMKnobAnimation : NSAnimation
{
    int start, range;
    id delegate;
}
@end
@implementation VMKnobAnimation
...
@implementation VMSwitcher

@synthesize delegate;

-(void)awakeFromNib
{
    surround = [NSImage imageNamed:@"button_surround"];
    knob = [NSImage imageNamed:@"button_knob"];
...
{
    VMKnobAnimation* a = [[VMKnobAnimation alloc] initWithStart:location.x end:x];
    [a setDelegate:self];
            ...
            switch ([localEvent type]) {
                ...
                case NSLeftMouseUp:
                    ...
                    if((location.x == 0 && state) || (location.x == KNOB_MAX_X && !state)){
                        state = !state;
                        [self.delegate didChangeState:self state:state ? NSOnState : NSOffState];
                    }
